/**
 * My Placements page logic with purchase modal
 */
...
async function loadActivePlacements() {
    try {
        ...
        renderActivePlacements(placements.filter(p => p.status === 'placed'));
...
async function loadScheduledPlacements() {
    try {
        ...
        renderScheduledPlacements(placements.filter(p => p.status === 'scheduled'));
