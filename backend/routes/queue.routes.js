...
router.get('/jobs', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (queueName && queues[queueName]) {
      ...
      const jobs = await queue.getJobs(['active', 'waiting', 'completed', 'failed'], 0, parseInt(limit));
      allJobs = jobs;
    } else {
...
// Cancel job
router.post('/jobs/:queueName/:jobId/cancel', asyncHandler(async (req, res) => {
  if (!queueService) {
    return res.status(503).json({
      error: 'Queue service not available'
    });
...
// Retry failed job
router.post('/jobs/:queueName/:jobId/retry', asyncHandler(async (req, res) => {
  if (!queueService) {
    return res.status(503).json({
      error: 'Queue service not available'
    });
