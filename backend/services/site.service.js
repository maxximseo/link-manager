...
const getSiteByDomain = async (domain) => {
  try {
    ...
    const result = await query(
      `SELECT id, user_id, site_name, site_url, site_type, max_links, max_articles, used_links, used_articles, allow_articles, dr, da, ref_domains, rd_main, norm, created_at
       FROM sites
       WHERE LOWER(
         REGEXP_REPLACE(
           REGEXP_REPLACE(
