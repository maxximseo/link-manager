...
const getSiteById = async (siteId, userId) => {
  try {
    const result = await query(
      'SELECT id, user_id, site_name, site_url, api_key, site_type, max_links, max_articles, used_links, used_articles, allow_articles, is_public, dr, da, ref_domains, rd_main, norm, tf, cf, keywords, traffic, geo, created_at FROM sites WHERE id = $1 AND user_id = $2',
      [siteId, userId]
    );
...
const getSiteByDomain = async (domain) => {
  try {
    ...
    const result = await query(
      `SELECT id, user_id, site_name, site_url, site_type, max_links, max_articles, used_links, used_articles, allow_articles, dr, da, ref_domains, rd_main, norm, tf, cf, keywords, traffic, created_at
       FROM sites
       WHERE LOWER(
         REGEXP_REPLACE(
           REGEXP_REPLACE(
