/**
 * Migration runner for adding DR (Domain Rating) column to sites table
 * Run with: node database/run_dr_migration.js
 */
...
async function runMigration() {
  ...
  try {
    // Read migration SQL
    const migrationPath = path.join(__dirname, 'migrate_add_dr.sql');
    ...
    if (verifyResult.rows.length > 0) {
      ...
      console.error('❌ Migration failed: DR column not found');
      process.exit(1);
    }
