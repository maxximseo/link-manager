...
async function testPlacementsManagerI18n() {
  ...
  try {
    ...
    await page.goto(`${CONFIG.baseUrl}/placements-manager.html`, { waitUntil: 'networkidle0', timeout: 15000 });
    ...
    const allLinksTab = await page.$eval('[data-i18n="allLinks"]', el => el.textContent).catch(() => 'NOT FOUND');
    const filterByProject = await page.$eval('[data-i18n="filterByProject"]', el => el.textContent).catch(() => 'NOT FOUND');

    console.log(`   Lang attribute: ${lang}`);
    console.log(`   All Links tab: ${allLinksTab}`);
    console.log(`   Filter by project: ${filterByProject}`);

    await page.screenshot({ path: path.join(CONFIG.screenshotDir, 'placements-manager-ru.png'), fullPage: true });
    ...
    if (allLinksTab.includes('Все') && filterByProject.includes('Фильтр')) {
      ...
      results.failed++;
      results.tests.push({ name: 'Russian Placements Manager Page', status: 'FAIL', allLinksTab, filterByProject });
      console.log('   FAIL\n');
    }
  } catch (error) {
    results.failed++;
    results.tests.push({ name: 'Russian Placements Manager Page', status: 'ERROR', error: error.message });
  ...
  try {
    ...
    const allLinksTab = await page.$eval('[data-i18n="allLinks"]', el => el.textContent).catch(() => 'NOT FOUND');
    const activeLinksTab = await page.$eval('[data-i18n="activeLinks"]', el => el.textContent).catch(() => 'NOT FOUND');
    const filterByProject = await page.$eval('[data-i18n="filterByProject"]', el => el.textContent).catch(() => 'NOT FOUND');
    ...
    await page.screenshot({ path: path.join(CONFIG.screenshotDir, 'placements-manager-en.png'), fullPage: true });
    console.log('   Screenshot saved: placements-manager-en.png');

    if (allLinksTab.includes('All') && activeLinksTab.includes('Active') && filterByProject.includes('Filter')) {
      results.passed++;
      results.tests.push({ name: 'English Placements Manager Page', status: 'PASS' });
      console.log('   PASS\n');
    } else {
      results.failed++;
      results.tests.push({ name: 'English Placements Manager Page', status: 'FAIL', allLinksTab, filterByProject });
      console.log('   FAIL\n');
    }
  } catch (error) {
    results.failed++;
    results.tests.push({ name: 'English Placements Manager Page', status: 'ERROR', error: error.message });
    console.log(`   ERROR: ${error.message}\n`);
  }
  ...
  try {
    const thProjectSite = await page.$eval('[data-i18n="thProjectSite"]', el => el.textContent).catch(() => 'NOT FOUND');
    const thPrice = await page.$eval('[data-i18n="thPrice"]', el => el.textContent).catch(() => 'NOT FOUND');
    const thActions = await page.$eval('[data-i18n="thActions"]', el => el.textContent).catch(() => 'NOT FOUND');

    console.log(`   Project/Site header: ${thProjectSite}`);
    console.log(`   Price header: ${thPrice}`);
    console.log(`   Actions header: ${thActions}`);

    if (thProjectSite.includes('PROJECT') && thPrice.includes('PRICE') && thActions.includes('ACTIONS')) {
      results.passed++;
  ...
  try {
    const allTypes = await page.$eval('[data-i18n="allTypes"]', el => el.textContent).catch(() => 'NOT FOUND');
    const dateFrom = await page.$eval('[data-i18n="dateFrom"]', el => el.textContent).catch(() => 'NOT FOUND');
    const dateTo = await page.$eval('[data-i18n="dateTo"]', el => el.textContent).catch(() => 'NOT FOUND');

    console.log(`   All types: ${allTypes}`);
    console.log(`   Date from: ${dateFrom}`);
    console.log(`   Date to: ${dateTo}`);

    if (allTypes.includes('All types') && dateFrom.includes('Date from') && dateTo.includes('Date to')) {
      results.passed++;
  ...
  try {
    const visibleColumns = await page.$eval('[data-i18n="visibleColumns"]', el => el.textContent).catch(() => 'NOT FOUND');
    const priceColumn = await page.$eval('[data-i18n="priceColumn"]', el => el.textContent).catch(() => 'NOT FOUND');
    const actionsColumn = await page.$eval('[data-i18n="actionsColumn"]', el => el.textContent).catch(() => 'NOT FOUND');

    console.log(`   Visible columns: ${visibleColumns}`);
    console.log(`   Price column: ${priceColumn}`);
    console.log(`   Actions column: ${actionsColumn}`);

    if (visibleColumns.includes('Visible') && priceColumn.includes('Price') && actionsColumn.includes('Actions')) {
      results.passed++;
  ...
  try {
    ...
    if (allLinksTab.includes('Все') && filterByProject.includes('Фильтр')) {
      ...
      results.failed++;
      results.tests.push({ name: 'Switch to Russian', status: 'FAIL', allLinksTab, filterByProject });
      console.log('   FAIL\n');
    }
  } catch (error) {
    results.failed++;
