...
async function testBalanceI18n() {
  ...
  try {
    ...
    await page.goto(`${CONFIG.baseUrl}/balance.html`, { waitUntil: 'networkidle0', timeout: 15000 });
    ...
    const currentBalanceLabel = await page.$eval('[data-i18n="currentBalanceLabel"]', el => el.textContent).catch(() => 'NOT FOUND');
    const totalSpentLabel = await page.$eval('[data-i18n="totalSpentLabel"]', el => el.textContent).catch(() => 'NOT FOUND');
    ...
    await page.screenshot({ path: path.join(CONFIG.screenshotDir, 'balance-ru.png'), fullPage: true });
    ...
    if (currentBalanceLabel.includes('Текущий') && totalSpentLabel.includes('Потрачено')) {
      ...
      results.failed++;
      results.tests.push({ name: 'Russian Balance Page', status: 'FAIL', currentBalanceLabel, totalSpentLabel });
      console.log('   FAIL\n');
    }
  } catch (error) {
    results.failed++;
  ...
  try {
    ...
    const currentBalanceLabel = await page.$eval('[data-i18n="currentBalanceLabel"]', el => el.textContent).catch(() => 'NOT FOUND');
    const depositBtn = await page.$eval('[data-i18n="depositBtn"]', el => el.textContent).catch(() => 'NOT FOUND');
    const totalSpentLabel = await page.$eval('[data-i18n="totalSpentLabel"]', el => el.textContent).catch(() => 'NOT FOUND');
    ...
    await page.screenshot({ path: path.join(CONFIG.screenshotDir, 'balance-en.png'), fullPage: true });
    console.log('   Screenshot saved: balance-en.png');

    if (currentBalanceLabel.includes('Current') && depositBtn.includes('Deposit') && totalSpentLabel.includes('Total')) {
      results.passed++;
      results.tests.push({ name: 'English Balance Page', status: 'PASS' });
      console.log('   PASS\n');
    } else {
      results.failed++;
      results.tests.push({ name: 'English Balance Page', status: 'FAIL', currentBalanceLabel, depositBtn });
      console.log('   FAIL\n');
    }
  } catch (error) {
    results.failed++;
  ...
  try {
    const yourDiscount = await page.$eval('[data-i18n="yourDiscount"]', el => el.textContent).catch(() => 'NOT FOUND');
    const discountTiersTitle = await page.$eval('[data-i18n="discountTiersTitle"]', el => el.textContent).catch(() => 'NOT FOUND');
    const goal = await page.$eval('[data-i18n="goal"]', el => el.textContent).catch(() => 'NOT FOUND');

    console.log(`   Your discount: ${yourDiscount}`);
    console.log(`   Discount tiers title: ${discountTiersTitle}`);
    console.log(`   Goal label: ${goal}`);

    if (yourDiscount.includes('discount') && discountTiersTitle.includes('Discount') && goal.includes('Goal')) {
      results.passed++;
  ...
  try {
    const transactionsHistory = await page.$eval('[data-i18n="transactionsHistory"]', el => el.textContent).catch(() => 'NOT FOUND');
    const thDate = await page.$eval('[data-i18n="thDate"]', el => el.textContent).catch(() => 'NOT FOUND');
    const thType = await page.$eval('[data-i18n="thType"]', el => el.textContent).catch(() => 'NOT FOUND');
    const thAmount = await page.$eval('[data-i18n="thAmount"]', el => el.textContent).catch(() => 'NOT FOUND');
    ...
    if (transactionsHistory.includes('Transaction') && thDate === 'Date' && thType === 'Type' && thAmount === 'Amount') {
      results.passed++;
  ...
  try {
    const tierLevel = await page.$eval('[data-i18n="tierLevel"]', el => el.textContent).catch(() => 'NOT FOUND');
    const tierMinAmount = await page.$eval('[data-i18n="tierMinAmount"]', el => el.textContent).catch(() => 'NOT FOUND');
    const tierDiscount = await page.$eval('[data-i18n="tierDiscount"]', el => el.textContent).catch(() => 'NOT FOUND');
    const tierStatus = await page.$eval('[data-i18n="tierStatus"]', el => el.textContent).catch(() => 'NOT FOUND');
    ...
    if (tierLevel === 'Tier' && tierMinAmount.includes('Minimum') && tierDiscount === 'Discount' && tierStatus === 'Status') {
      results.passed++;
  ...
  try {
    ...
    if (currentBalanceLabel.includes('Текущий') && transactionsHistory.includes('История')) {
      ...
      results.failed++;
      results.tests.push({ name: 'Switch to Russian', status: 'FAIL', currentBalanceLabel, transactionsHistory });
      console.log('   FAIL\n');
    }
  } catch (error) {
    results.failed++;
